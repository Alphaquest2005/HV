                    ...
                    using (var ctx = new RMSModel())
                    {
                        _pharmacists =
                            new ObservableCollection<Cashier>(
                                ctx.Persons.OfType<Cashier>().Where(x => x.Role == "Pharmacist" && x.IsActive == true));
        ...
        private TransactionBase GetDBTransaction(int transactionId, string type)
        {
            try
            {
                ...
                using (var ctx = new RMSModel())
                {
                    ...
                    if (res.ParentTransactionId.HasValue)
                    {
            ...
            using (var ctx = new RMSModel())
            {
                try
                {
                    ...
                    if (trans.TransactionId == 0)
                    {
                        ...
                        VALUES        ({trans.StationId},'{trans.ParentTransactionId}',{trans.BatchId},{trans.CloseBatchId},'{trans.Time}','{trans.CustomerId}',{trans.PharmacistId},{trans.CashierId},'{trans.Comment?.Replace("'","''")}','{trans.ReferenceNumber}','{trans.StoreCode}',{trans.OpenClose},'{trans.Status?.Replace("'", "''")}','{trans.DeliveryType}')";
                        sql = CleanSql(sql);
