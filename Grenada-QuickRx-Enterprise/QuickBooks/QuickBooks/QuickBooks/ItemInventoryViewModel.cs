...
namespace QuickBooks
{
    ...
    {
        private static DateTime lastDownloadTime = DateTime.MinValue;
        public void BuildModifiedItemInventoryQuery(IMsgSetRequest ItemInventoryRequestMsgSet, int days)
        {
            if (days != 1) lastDownloadTime = DateTime.Now.AddDays(days*-1);
            if (ItemInventoryRequestMsgSet == null) return;
            ...
            if (lastDownloadTime != DateTime.MinValue)
            {
                ...
                ItemInventoryQueryRq.ORTimeModifiedFilters.TimeModifiedRangeFilter.FromTimeModified.SetValue(DateTime.Now.AddMinutes(-45),//lastDownloadTime
                    false);
                ItemInventoryQueryRq.ORTimeModifiedFilters.TimeModifiedRangeFilter.ToTimeModified.SetValue(
                    DateTime.Now, false);
        ...
        public async Task<Tuple<string, TrackableCollection<ItemInventoryRet>>> WalkItemInventoryQueryRsAsync(IMsgSetResponse responseMsgSet)
        {
            var t = new TrackableCollection<ItemInventoryRet>(null);
            if (responseMsgSet == null)
            {

                return new Tuple<string, TrackableCollection<ItemInventoryRet>>("0", t);
            }
                ...
                if (response.StatusCode >= 0)
                {
                    ...
                    if (response.Detail != null)
                    {
                        ...
                        if (responseType == ENResponseType.rtItemInventoryQueryRs)
                        {
                            ...
                            t = WalkItemInventoryRet(ItemInventoryRet);
                           // return new Tuple<string, TrackableCollection<ItemInventoryRet>>("Complete", t);
                        }
                    }
                    else
                    {
      ...
      public  QuickBooks.TrackableCollection<ItemInventoryRet> WalkItemInventoryRet(IItemInventoryRetList ItemInventoryRetList)
        {
            ...
            for (int i = 0; i < ItemInventoryRetList.Count; i++)
            {
                ...
                if(positm.UnitOfMeasure != null)
                    itm.UnitOfMeasure = (string)positm.UnitOfMeasure.GetValue();

                if (positm.QuantityOnHand != null)
                    itm.QuantityOnHand = (int) positm.QuantityOnHand.GetValue();
        ...
        public void BuildItemInventoryQueryRq(IMsgSetRequest ItemInventoryRequestMsgSet, int FromItemNumber,
            int ToItemNumber)
        {
