    ...
    public class QBPOS //: IDisposable
    {
        ...
        private static QBResult GetQBResult(string response)
        {
            ...
            foreach (XmlNode itm in vendRetNodeList)
            {
                ...
                if (res.SalesReceiptNumber != null && res.TrackingNumber != null) break;
            }
            return res;
        }
            ...
            try
            {
                ...
                foreach (XmlElement itm in inventoryItemRsNodeList)
                {
                    ...
                    foreach (XmlNode xi in qbitms)
                    {
                        ...
                        {
                            i.CustomerDiscType = (xi.InnerText == "None" ? Convert.ToByte(0) : Convert.ToByte(xi.InnerText));
                            continue;
                        }
                        ...
                        XmlNodeList BillAddress = xi.SelectNodes("./BillAddress");
                        if (BillAddress != null)
                        {
                            if (xi.SelectSingleNode("./BillAddress/Street") != null)
                            {
                                i.BillAddress.Street = xi.SelectSingleNode("./BillAddress/Street").InnerText;
                            }
                        }
